...
Beide Launch-Sequenzen bedingen eine weitreichende Integration von Autorisierungsserver und App-Laucher, die gemeinsam den Launch-Kontext des ISiK-Clients bestimmmen und verwalten. Diese enge Bindung steht im Widerspruch zu den formulierten [Vorgaben für ISiK-Sicherheit in ISiK Stufe 3](Motivation.md) und erschwert die Nutzung von marktgängigen Produkten wie z. B. _Keycloak_, _Auth0_ und/oder _NGINX_ zum Aufbau eines IAM-Subsystems. Daher werden in ISiK Stufe 3 im Modul ISiK-Sicherheit zunächst nur Interaktionen gegen Systeme in der Rolle eines ISiK-Ressourcenservers definiert. Die Vorgaben umfassen dabei insbesondere die SMART-Konfiguration (Schritt 3) sowie die Syntax und Semantik der über das Zugriffstoken vermittelten Autorisierungen (Schritt 6). 
...
ISiK-Autorisierung nutzt für die Kodierung und Durchsetzung von Autorisierungen drei Konzepte aus FHIR bzw. SMART-on-FHIR:
* Kontext
* Compartment 
* Scope
...
ISiK-Autorisierung in der ISiK Stufe 3 verlangt von FHIR-Ressourcenservern, dass diese zumindest Autorisierungen mit Bezug zu der in FHIR definierten _Compartment_-Definition für patientenbezogene Daten verarbeiten können.
